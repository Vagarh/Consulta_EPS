## conexion base de datos savia contrtos tarifas, medicamentos y tencologias
SELECT DISTINCT  
    cp.numero_documento, 
    cp.razon_social,
    c.contrato,
    ...
    cd.ma_tecnologia_codigo AS codigo_tecnologia,
    cd.ma_tecnologia_valor ,
    cd.valor_manual ,
    cd.valor_contratado,
    cd.complejidad ,
    cs.mae_modalidad_contrato_valor ,
    gu.nombre ,
    gu.mae_region_valor 
...
left join gn_ubicaciones gu on gu.id = cps.ubicacion_id 
  where cd.activo = '1' and cd.tipo_tecnologia in ('1','4','3','2')